...
function ImgMgr() {
  ...
  var frameDelay = 10;
  var nFrames = nCols * frameDelay; //4 * frameDelay; //numImgInGrid * frameDelay;
  var rFrameOffset = [];
  for (i=0;i<5;i++){
    rFrameOffset.push( 0 );
  }
    ...
    for ( gx = 0; gx<width/gridSize; gx+=1 ) {
      for ( gy = 0; gy<height/gridSize; gy+=1 ) {
        if ( !gridPointAngle[gridIndex] ) {
          var randomAngle = 0;//PI/2 * floor(random(8));
          gridPointAngle[gridIndex] = randomAngle;
          gridPointScale[gridIndex] = random(1,3);
        }
        drawFrameAtIndexToGrid(gx*frameDelay+gy/nRows*frameDelay, gx, gy, gridPointAngle[gridIndex], gridPointScale[gridIndex] );
        gridIndex += 1;
      }
    }
...
function SourceImage(sizeIn) {
  ...
  var lightSize = size*0.8;
  var lightColor = color(200,200,0);

  var lightX=0;
  var lightY=0;
  var shadowOffsetX = 0;
  var shadowOffsetY = 0;
  var shadowOffsetFromX = 0;
  var shadowOffsetFromY = 0;
  var shadowOffsetToX = 0;
  var shadowOffsetToY = 0;

  var isMovingShadow = false;
  var startMovingShadowAt = millis();
  var shadowMovementDuration = 5000;

  var moveShadow = function() {
    if ( !isMovingShadow ) {
      if ( millis() >= startMovingShadowAt ) {
        isMovingShadow = true;
        if ( abs(shadowOffsetX) < lightSize/2 ) {
          shadowOffsetToX = shadowOffsetFromX < 0 ? lightSize : -lightSize;
          shadowOffsetFromX = 0;
        } else {
          shadowOffsetFromX = shadowOffsetX;
          shadowOffsetToX = -shadowOffsetX;
        }
      }
    }
    if ( isMovingShadow ) {
      shadowOffsetX = Math.easeInOutCubic( millis()-startMovingShadowAt, shadowOffsetFromX, shadowOffsetToX, shadowMovementDuration );
      if ( millis() > startMovingShadowAt + shadowMovementDuration ) {
        isMovingShadow = false;
        startMovingShadowAt = millis();// + shadowMovementDuration/10;
      }
    }
  };
  ...
  var drawGraphics = function() {
    ...
    g.fill(lightColor);
    g.ellipse(lightX,lightY,lightSize,lightSize);

    g.fill(0);
    g.ellipse(lightX+shadowOffsetX,lightY+shadowOffsetY,lightSize,lightSize);

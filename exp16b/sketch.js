...
function ShapeMgr() {
  ...
  var reset = function() {
    ...
    if ( width > height ) {
      ...
      _self._standardGridPointSize = new p5.Vector(floor(width/numGridPointsAcross), floor(width/numGridPointsAcross));
      //_self._standardGridPointSize = new p5.Vector(gridHeight/numGridPointsDown, gridHeight/numGridPointsDown);
       ...
       _self._standardGridPointSize = new p5.Vector(floor(height/numGridPointsDown), floor(height/numGridPointsDown));
...
function Line( mgr ) {
  ...
  var _xy = function( gx, gy ) {
    ...
    var xx = floor(gx*ps.x + (width-_widthGridAcross())/2);
    var yy = floor(gy*ps.y + (height-_heightGridDown())/2);
  ...
  var _drawAt = function( gx, gy ) {
    ...
    var xy = _xy(gx, gy);
    var offsetFraction = _pixelSizeOffset.x;
    var pixelSizeOffset = xy.ps.x * offsetFraction;
    var pixelWidth = floor(xy.ps.x-pixelSizeOffset);
    var pixelHeight = floor(xy.ps.y-pixelSizeOffset);
    push();
    fill(0);
    rect( xy.x, xy.y, xy.ps.x, xy.ps.y );
    pop();
    rect( xy.x, xy.y, pixelWidth, pixelHeight );
  };
  ...
  var _reset = function() {
    ...
    var startingPixelOffsetFraction;
    var endingPixelOffsetFraction;
    var r = random(4);
    if ( r < 1 ) {
      startingPixelOffsetFraction = 0.05;
      endingPixelOffsetFraction = 0.05;
    } else if ( r < 2 ) {
      startingPixelOffsetFraction = 0.5;
      endingPixelOffsetFraction = 0.5;
    } else if ( r < 3 ) {
      startingPixelOffsetFraction = 0.5;
      endingPixelOffsetFraction = 0.5;
    } else {
      startingPixelOffsetFraction = 0.2;
      endingPixelOffsetFraction = 0.2;
    }
    _pixelSizeOffset = new p5.Vector(startingPixelOffsetFraction,1);
    if ( startingPixelOffsetFraction !== endingPixelOffsetFraction ) {
      var t1 = new TWEEN.Tween(_pixelSizeOffset)
        .to({x:endingPixelOffsetFraction},5000);
      var t2 = new TWEEN.Tween(_pixelSizeOffset)
        .to({x:1},5000);
      t1.chain( t2 );
      t2.chain( t1 );
      t1.start();
